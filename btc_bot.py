...
communication. All trading intelligence lives in backtest_btc.py — this
...
bot_logger = logging.getLogger("btc_bot")
error_logger = logging.getLogger("btc_bot.errors")
...
@flask_app.route("/")
def home():
    return "BTC signal bot is alive.", 200
...
def format_analysis_message(result):
    ...
    lines = [f"{emoji} *BTC/USDT — {signal}*", f"💰 Price: ${result['price']:,.2f}"]
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is not None:
        ...
        await update.message.reply_text("⚠️ Couldn't fetch the current BTC price. Try again shortly.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "start":
        ...
        price = with_retries(engine.fetch_spot_price, max_retries=2)
        text = f"₿ BTC/USDT: ${price:,.2f}" if price is not None else "⚠️ Couldn't fetch price."
...
def main():
    ...
    bot_logger.info("BTC/USDT bot starting...")
