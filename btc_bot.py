...
-----------------------------
Price source: Coinbase public spot price endpoint (BTC-USD, no API key required)
...
VOLUME_CONFIRMATION_MULTIPLIER = 1.1   # current volume must exceed avg * this to confirm (was 1.2 — loosened)
...
RSI_BUY_CEILING = 60
RSI_SELL_FLOOR = 40
CONFIRMATION_STREAK = 4    # 4 * 15min = still 60 min sustained trend — same duration as
                           # before, just checked twice as often now that polling is faster
...
# ---------------------------------------------------------------------------
def fetch_price():
    """Fetch the latest BTC/USD spot price from Coinbase. Returns float or None.
