...
backtest_btc.py
----------------
Purpose: contains all trading intelligence and market analysis for the BTC
bot. btc_bot.py handles Telegram/scheduling/infra and calls analyze() here
...
logger = logging.getLogger("backtest_btc")
...
def fetch_spot_price():
    """Latest BTC/USD spot price from Coinbase. Returns float or None."""
    try:
...
def fetch_candles(granularity, limit=200):
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "btc-analysis-engine/1.0"},
            timeout=15,
        )
        resp.raise_for_status()
        raw = resp.json()
...
EXTENSION_ATR_MULTIPLE = 2.0  # price this many ATRs away from EMA20 counts as "already stretched"


def is_overextended(raw_direction, price, indicators):
    """
    Structure confirmation (fractal swings) lags the actual pivot by a couple
    candles, so a fresh BULLISH/BEARISH read can arrive after the move has
    already run — firing a BUY right as the move exhausts (buying the top),
    or a SELL right as it exhausts to the downside (selling the bottom).
    This checks whether price has already stretched too far from EMA20 in
    ATR terms; if so, the entry is chasing, not catching, the move.
    """
    if raw_direction not in ("BUY", "SELL"):
        return False
    ema20, atr_val = indicators.get("ema20"), indicators.get("atr")
    if ema20 is None or not atr_val:
        return False
    distance = (price - ema20) if raw_direction == "BUY" else (ema20 - price)
    return distance > EXTENSION_ATR_MULTIPLE * atr_val
...
# ---------------------------------------------------------------------------
# Final AI Decision Engine — the single entry point btc_bot.py calls
# ---------------------------------------------------------------------------

def analyze():
    """
    Orchestrates every section above into one trading decision. Returns a
    dict ready for btc_bot.py's message formatting, or None if live data
    ...
    raw_direction, _ = decide_trade(structure, mtf, reversal)
    if is_overextended(raw_direction, price, indicators):
        raw_direction = "WARNING"
...
if __name__ == "__main__":
    # Quick manual smoke test: `python backtest_btc.py`
