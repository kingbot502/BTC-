"""
BTC Bot Backtester (Full Logic + P&L Simulation)
------------------------------------------------------------------------
Replays the EXACT current btc_bot.py signal pipeline against real historical
data:
...
HOW TO RUN:
  ...
  2. python backtest_btc.py
  3. Results print to console, plus a full per-signal CSV
     (backtest_btc_results.csv) including confidence AND per-trade P&L.
"""
...
# ---------------------------------------------------------------------------
# Config — mirrors btc_bot.py exactly
# ---------------------------------------------------------------------------
RSI_PERIOD = 15
MA_SHORT = 5
MA_MED = 10
...
# ---------------------------------------------------------------------------
# Indicator logic — copied verbatim from btc_bot.py
# ---------------------------------------------------------------------------
...
def run_backtest():
    ...
    for idx, sig, entry_price, confidence in signals:
        ...
        results_log.append(("BTC/USDT bot (full logic)", idx, sig, round(entry_price, 4),
    ...
    print(f"\n=== BTC/USDT bot — signal accuracy (direction only, no fees) ===")
    ...
    print(f"\n=== BTC/USDT bot — P&L simulation (fees, slippage, position sizing) ===")
    ...
    if results_log:
        import csv
        with open("backtest_btc_results.csv", "w", newline="") as f:
            writer = csv.writer(f)
        ...
        print("\nFull signal-by-signal log (including P&L per trade) saved to backtest_btc_results.csv")
